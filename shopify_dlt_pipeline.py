...
def load_all_resources(resources: List[str], start_date: TAnyDateTime) -> None:
    ...
    pipeline = dlt.pipeline(
        pipeline_name="shopify", destination='redshift', dataset_name="shopify_bodylabch_dlthub"
    )
...
def incremental_load_with_backloading() -> None:
    ...
    pipeline = dlt.pipeline(
        pipeline_name="shopify", destination='redshift', dataset_name="shopify_data"
    )
...
def load_partner_api_transactions() -> None:
    ...
    load_info = pipeline.run(resource, verbose=True)
    print(load_info)
